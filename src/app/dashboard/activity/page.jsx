...
const page = () => {
  ...
  return (
    <div className="flex-1 bg-[#0B0F19] h-screen text-white px-6 py-8 w-full overflow-y-auto text-sm sm:text-base">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-[#111827] border-blue-600 border-1 p-6 rounded-2xl shadow-lg mb-6">
        ...
        <div className="flex space-x-2 items-center">
          ...
          <div className="flex items-center space-x-4 mt-4 md:mt-0">
            <select
              className="block w-full bg-gray-800 text-sm sm:text-base text-white px-2 py-2 rounded-md border border-white/20 focus:outline-none"
      ...
      <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0 md:space-x-4">
        <div className="flex flex-wrap justify-center md:justify-end items-center mt-4 space-x-1">
          <button
            ...
            className={`px-3 py-1 rounded-md text-xs sm:text-sm ${currentPage === 1
            ...
            .map((page, idx, arr) => (
              <React.Fragment key={page}>
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-xs sm:text-sm ${currentPage === page
          ...
          <button
            ...
            className={`px-3 py-1 rounded-md text-xs sm:text-sm ${currentPage === validTotalPages
                ? 'bg-[#1F2937] text-gray-500 cursor-not-allowed'
                : 'bg-[#1F2937] text-white hover:bg-[#374151]'
              }`}
          >
      ...
      {showModal && selectedImage && (
        <div className="fixed inset-0 z-50 bg-black/5 bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-xl w-11/12 lg:w-3/4 xl:w-2/3 shadow-lg max-h-[90vh] overflow-y-auto relative text-sm sm:text-base">
            <button
              ...
              <div className="flex flex-col justify-start">
                <div className="mb-6">
                  <label className="flex items-center cursor-pointer">
                    <span className="mr-3 font-semibold text-gray-800 text-sm sm:text-base">Show Original</span>
                    <input
                      type="checkbox"
                      className="sr-only"
                ...
                <div className="mb-4">
                  <h2 className="text-base sm:text-lg font-semibold mb-3 text-gray-800">Predictions</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                    {selectedImage.predictions.map((pred, i) => (
                      <div
                        key={i}
                        className="p-2 bg-gray-200 rounded-lg w-fit text-xs sm:text-sm text-black"
                      >
                ...
                {selectedImage.note && (
                  <div>
                    <h2 className="text-base sm:text-lg font-semibold mb-3 text-gray-800">My Note</h2>
                    ...
                    {selectedImage.note.length > 200 && (
                      <button
                        className="mt-1 text-blue-600 hover:underline text-xs sm:text-sm"
